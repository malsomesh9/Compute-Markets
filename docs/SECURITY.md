...
Off-chain: RLS and grants restrict private tables and all projection writes. Server-owned tables use explicit restrictive deny policies for browser roles, while the public registry tables grant anonymous SELECT only. Wallet ownership proof is required for private job API access. InsForge's storage RLS is explicitly enabled; evidence is accessible through the authorized API/admin path. The browser receives only an anon key. JSON bodies and evidence uploads are bounded; no arbitrary input URL is fetched. The hosted InsForge advisor currently reports zero critical and zero warning findings; its seven public-read findings are suppressed individually with recorded justification so unrelated future policies remain visible.
