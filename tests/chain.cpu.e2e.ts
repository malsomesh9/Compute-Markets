...
process.env.MOCK_EXECUTOR = "false";
...
const executor = new DockerBackend({
  runtime: ociRuntime(process.env.OCI_RUNTIME),
});
try {
  await executor.prepare(job.toBase58(), spec, []);
  await send(winner.wc, "startJob", [], {
    worker: winner.worker.publicKey,
    job,
    workerAuth: winner.workerAuth,
  });
  const started = await account(c, "job", job);

  await executor.start(job.toBase58());
  const result = await executor.collectResult(job.toBase58());
  const executionSeconds = Math.max(0, result.finishedAt - result.startedAt);
  const receiptStart = Number(started.startedAt);
  const telemetry = {
    ...result.telemetry,
  ...
  const payload = {
    ...
    startTimestamp: receiptStart,
    finishTimestamp: receiptStart + executionSeconds,
  ...
  assert.equal(
    (await getAccount(c.connection, winner.payout.address)).amount -
      payoutBefore,
    34300n,
  );
  assert.equal(
  ...
  writeFileSync(
    process.env.OCI_RUNTIME === "runsc"
      ? "research/e2e-gvisor-result.json"
      : "research/e2e-cpu-result.json",
    JSON.stringify(
      {
        network: "localnet",
        execution: `real Docker OCI (${process.env.OCI_RUNTIME ?? "runc"}), non-root, network-denied, read-only CPU workload`,
        ociRuntime: process.env.OCI_RUNTIME ?? "runc",
        program: c.program.programId.toBase58(),
        job: job.toBase58(),
        mint: mint.toBase58(),
        settlementTx,
  ...
  await executor.cleanup(job.toBase58()).catch(console.error);
}
